...
import { getObjectBuffer } from "@/lib/image-translate/oss";
...
export async function GET(req: NextRequest, { params }: Props) {
  ...
  if (task.items.length === 1) {
    const { signedUrl } = await import("@/lib/image-translate/oss");
  ...
  return new NextResponse(new Uint8Array(zipBuffer), {
    headers: {
      ...
      "Content-Disposition": `attachment; filename="image-translate-${id.slice(0, 8)}.zip"`,
    },
  });
}
