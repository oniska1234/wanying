...
class OSSClient:
    ...
    def ensure_lifecycle(self) -> bool:
        """Ensure an OSS lifecycle rule exists for this service prefix."""
        if not self._bucket:
            return False
        try:
            from oss2.models import LifecycleRule, LifecycleExpiration, BucketLifecycle
            rule_id = f"{OSS_PREFIX.replace('/', '-')}-expire-90d"
            try:
            ...
            LOGGER.info("OSS lifecycle rule ensured: %s/ expires in 90 days", OSS_PREFIX)
            return True
